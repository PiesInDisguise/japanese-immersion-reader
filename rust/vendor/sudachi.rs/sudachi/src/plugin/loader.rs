...
// Upstream (v0.6.11) doesn't list target_os = "android" here even though
// Android's shared-library naming/extension is identical to Linux's --
// `cfg(target_os)` treats "android" as distinct from "linux" (unlike
// `cfg(target_family = "unix")`, which covers both), so without this the
// crate fails to compile for Android at all. See rust/vendor/README.md.
